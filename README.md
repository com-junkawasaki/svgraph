...
- DrawingML: preset geometry shapes including common polygon, arrow, symbol, and flowchart presets, custom geometry paths including line/quadratic/cubic segments, native table cells and simple merged cells under `p:graphicFrame` as editable SVG rectangles/text, and text boxes under `p:sp`, with invalid numeric DrawingML attributes ignored where possible
